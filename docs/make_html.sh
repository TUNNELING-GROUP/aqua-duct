...
sphinx-apidoc -f -e -o source/ ../src/aquaduct/
...
echo "* Python 3.9 (CPython implementation)" > source/aquaduct_install_requires.rst
printf 'install_requires_nice(1)' | ${PYTHON} -i ../src/setup.py -n --name 2> /dev/null > source/aquaduct_install_requires.rst

# valve HELP
sed '1,/HELP/!d' source/valve/valve_manual.rst.template | sed '$ d' > source/valve/valve_manual.rst
${PYTHON} ../src/apps/valve.py -h 2> /dev/null | awk '{print "    "$0}' >> source/valve/valve_manual.rst
...
${PYTHON} ../src/apps/pond.py -h 2> /dev/null | awk '{print "    "$0}' >> source/pond/pond_manual.rst
