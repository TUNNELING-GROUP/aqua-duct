...
def are_points_within_convexhull_old(points,chull,map_fun=None,sane_huge=10000):
    return np.array([is_point_within_convexhull((p,chull)) for p in points])
    '''
    ...
    while sum(ids)>N:
        ...
        if is_point_within_convexhull((p[:3],chull)):
            ...
            for dfacet,simp,equa in zip(dfacets,chull.simplices_vertices,chull.equations):
                dmin_vertices = 0
                for nr in range(3):
                    t = list(map(float,[dfacet[nr],dfacet[(nr+1)%3],dver[simp[nr],simp[(nr+1)%3]]]))
...
if __name__ == "__main__":
    ...
    for nr in range(10):
        ...
        with tictoc("Normal",stdout=True):
            ...
            with tictoc("old_way"):
                old_way = np.array([is_point_within_convexhull((p,chull)) for p in B])
            with tictoc("new_way"):
