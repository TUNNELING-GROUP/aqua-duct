...
def stage_I_run(config, options,
    ...
    pool = [Process(target=stage_I_worker_q,args=(input_queue,results_queue,pbar_queue)) for dummy in range(optimal_threads.threads_count)]
...
def stage_II_worker_q(input_queue,results_queue,pbar_queue):
    ...
    for input_data in iter(input_queue.get,None):
        ...
        paths_this_layer = (GenericPaths(resid,
                            ...
                            for resid, resname in zip(all_res_this_ids,
        ...
        for rid_in_object, frame in zip(iterate_or_die(frame_rid_in_object, times=number_of_frames),
            ...
            number_frame_object_scope[frame, :] = np.array(list(map(sum, zip(is_res_in_object, is_res_in_scope))), dtype=np.int8)
        ...
        paths = []
        for pat,nfos in zip(paths_this_layer,number_frame_object_scope.T):
            pat.add_012(nfos)
            paths.append(pat)
...
def stage_II_run(config, options,
    ...
    if number_frame_rid_in_object is None and res_ids_in_object_over_frames is not None:
        with clui.fbm("Get number_frame_rid_in_object if possible"):
            number_frame_rid_in_object = []
            for number in range(max(res_ids_in_object_over_frames.keys())):
                frame_rid_in_object = []
                for layer in range(max(res_ids_in_object_over_frames[number].keys())):
    ...
    # prepare and start pool of workers
    pool = [Process(target=stage_II_worker_q,args=(input_queue,results_queue,pbar_queue)) for dummy in range(optimal_threads.threads_count)]
    [p.start() for p in pool]

    # feed input_queue with data
    for results_count,(frame_rid_in_object, (number, traj_reader)) in enumerate(zip(iterate_or_die(number_frame_rid_in_object,
...
def stage_III_run(config, options,
    ...
    if options.discard_short_paths or options.discard_short_object:
        ...
        if short_paths is not None or short_object is not None:
            with clui.pbar(len(spaths),discard_message) as pbar:
                ...
                n = max(1,optimal_threads.threads_count)
                spaths_new = pool.imap_unordered(dse,(spaths[i:i + n] for i in range(0, len(spaths), n)))
                # CRIC AWARE MP!
                if short_object is not None:
    ...
    if options.auto_barber:
        ...
        with clui.pbar(maxval=len(paths),mess="AutoBarber in action:") as pbar:
            ...
            n = max(1, optimal_threads.threads_count)
            paths_new = pool.imap_unordered(bp, (paths[i:i + n] for i in range(0, len(paths), n)))
            paths_ = []
